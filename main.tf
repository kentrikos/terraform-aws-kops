...
resource "null_resource" "kubernetes_cluster-cross-account" {
  ...
  provisioner "local-exec" {
    when    = "destroy"
    command = "${path.module}/local-exec/kentrikos_k8s_cluster_deploy.sh --cluster-name-prefix ${var.cluster_name_prefix} --region ${var.region} --assume-cross-account-role ${var.iam_cross_account_role_arn} --action destroy"
    working_dir = "kops"
  }
}
